...
{	
    ...
    protected $_options = array(
        'file_name_prefix' => 'cache__',
    	'mode'            => 1, //mode 1 为serialize model 2为保存为可执行文件
    );
    
    public function __construct(){
        $this->_options['cache_dir'] = ROOT.'/runtime/cache/';
    	!file_exists($this->_options['cache_dir']) && xmkdir($this->_options['cache_dir']);
    }
	...
	protected function _fileNameToId($fileName)
	{
		$instance  = self::getInstance();
		$prefix    = $instance->_options['file_name_prefix'];
		return preg_replace('/^' . $prefix . '___(.*)$/', '$1', $fileName);
	}
	...
	{
		$instance  = self::getInstance();
		$glob = @glob($instance->_options['cache_dir'] . $instance->_options['file_name_prefix'] . '__*');
